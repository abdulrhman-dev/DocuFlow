CREATE TYPE "public"."role" AS ENUM('professor', 'department_manager', 'administrator', 'reviewer', 'director');--> statement-breakpoint
CREATE TYPE "public"."request_status" AS ENUM('draft', 'pending', 'approved', 'rejected');--> statement-breakpoint
CREATE TYPE "public"."instance_status" AS ENUM('in_progress', 'completed', 'printed', 'approved', 'rejected');--> statement-breakpoint
...
CREATE TABLE "Students" (
	...
	"createdAt" timestamp with time zone DEFAULT now() NOT NULL,
	"updatedAt" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "Students_nationalId_unique" UNIQUE("nationalId")
);
--> statement-breakpoint
...
CREATE TABLE "RequestAssignments" (
	...
	CONSTRAINT "request_assignments_month_range" CHECK ("RequestAssignments"."month" IS NULL OR ("RequestAssignments"."month" BETWEEN 1 AND 12)),
	CONSTRAINT "request_assignments_year_range" CHECK ("RequestAssignments"."year" IS NULL OR ("RequestAssignments"."year" BETWEEN 1900 AND 3000))
);
--> statement-breakpoint
...
ALTER TABLE "WorkflowInstances" ADD CONSTRAINT "WorkflowInstances_rejectedAtStageId_Stages_id_fk" FOREIGN KEY ("rejectedAtStageId") REFERENCES "public"."Stages"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "WorkflowInstances" ADD CONSTRAINT "WorkflowInstances_printedById_Users_id_fk" FOREIGN KEY ("printedById") REFERENCES "public"."Users"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "WorkflowInstances" ADD CONSTRAINT "WorkflowInstances_approvedById_Users_id_fk" FOREIGN KEY ("approvedById") REFERENCES "public"."Users"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
